...
let WILL = {
	...
	init(deviceModel, layoutBridge) {
		...
		this.VIEW_AREA = Module.RectTools.create(
			0,
			0,
			deviceModel.size.width,
			deviceModel.size.height
		);
	...
	initEvents() {
		...
		this.events = {
			resize: (e) => {
				if (this.layoutBridge.liveMode) this.context2D.fitToScreen();
			},
			...
			wheel: (e) => {
				...
				if (this.lastNotification)
					WILL.removeNotification(this.lastNotification);
				this.lastNotification = "zoom" + Math.random();
				WILL.addNotification({
					key: this.lastNotification,
					message: `${Math.floor(
						this.context2D.transformScaleFactor * 100
					)}%`,
				});
			},
				...
				if (!stroke.isBezierPath()) {
					if (!this.bezierProgress) {
						this.bezierProgress = {
							...
							length: [].concat.apply(
								[],
								this.layers.map((layer) => layer.strokes)
							).length,
						};
						this.layoutBridge.startProgress();
					}

					this.messanger.sendStrokePath(
						stroke,
						layerIndex,
						strokeIndex
					);
				} else
					dirtyArea = Module.RectTools.union(
						dirtyArea,
						stroke.bounds
					);
			});

			if (dirtyArea) this.context2D.refresh(dirtyArea, layerIndex);
		});

		if (!this.bezierProgress) this.refreshLayers();
	},
	...
	updateBezierProgress(stroke, layerIndex, strokeIndex) {
		...
		this.layoutBridge.updateProgress(
			this.bezierProgress.count * (100 / this.bezierProgress.length)
		);
	},
	...
	refreshLayers(affectedLayers) {
		...
		this.layers.forEach((layer, layerIndex) =>
			this.context2D.refresh(this.VIEW_AREA, layerIndex)
		);
	...
	setSplitMode(splitMode, canceled) {
		...
		if (splitMode) {
			...
			if (split.mode == "layer") {
				...
				if (!canceled)
					this.layoutBridge.updatePreview(
						split.affectedLayer,
						split.affectedLayer + 1
					);
			...
			} else throw new Error("Unknown split mode found: " + split.mode);
		}
	},
	...
	setSplitIndex(splitIndex) {
		...
		if (this.split.prev.index != this.split.index) {
			...
			if (this.split.mode == "layer")
				...
				strokes = [].concat.apply(
					[],
					this.layers.map((layer) => layer.strokes)
				);

			if (strokes) {
				let affectedStrokes = strokes.slice(
					Math.min(this.split.prev.index || 0, splitIndex),
					Math.max(this.split.prev.index || 0, splitIndex)
				);
				let affectedArea;

				affectedStrokes.forEach((stroke) => {
					affectedArea = Module.RectTools.union(
						affectedArea,
						stroke.bounds
					);
				});
				...
				if (affectedArea) {
					if (this.split.mode == "layer")
						...
						if (
							strokeIndex == 0 &&
							affectedLayer > 0 &&
							this.split.prev.index < splitIndex
						) {
							affectedLayer--;
							strokeIndex =
								this.layers[affectedLayer].strokes.length;
						}

						if (affectedLayer > this.split.prev.affectedLayer) {
							for (
								let i = this.split.prev.affectedLayer;
								i <= affectedLayer;
								i++
							) {
								if (i == affectedLayer)
								...
								else this.context2D.refresh(affectedArea, i);
							}
						} else
							this.context2D.refresh(
								affectedArea,
						...
						throw new Error(
							"Unknown split mode found: " + this.split.mode
						);
				}
			}
		}
	...
	initInkEngine() {
		...
		this.canvas = new Module.InkCanvas(
			...
			maxDeviceModelSize.width,
			maxDeviceModelSize.height
		);
		this.strokesLayer = this.canvas.createLayer(maxDeviceModelSize);
		this.strokesAndCurrentStrokeLayer =
			this.canvas.createLayer(maxDeviceModelSize);
	...
	setColorFromHex(color) {
		this.color = Module.Color.fromHex(color);
	...
	enableStrokeInput() {
		if (!this.strokeInputEnabled) {
			this.canvas.surface.addEventListener(
				"pointerdown",
	...
	disableStrokeInput() {
		if (this.strokeInputEnabled) {
			this.canvas.surface.removeEventListener(
				"pointerdown",
				this.events.pointerdown
			);
	...
	getOffset(e) {
		...
		let offsetRect =
			WILL.context2D.layersNode.offsetParent.getBoundingClientRect();
		let pt = Module.MatTools.transformPoint(
			{ x: e.pageX - offsetRect.x, y: e.pageY - offsetRect.y },
			Module.MatTools.invert(WILL.context2D.transform)
		);
	...
	beginStroke(point) {
		...
		if (!this.isInEraseMode()) this.drawPath();
		else this.erase();
	},
		...
		let cahcePointerInput = () => {
			if (this.inputPhase && this.inputPhase == Module.InputPhase.Move) {
				if (this.layoutBridge.liveMode) {
					...
					this.pointerChunk = [];
				} else this.buildPath(this.pointerPos);
	...
	buildPath(point) {
		if (this.inputPhase == Module.InputPhase.Begin) {
			if (!this.layoutBridge.liveMode) this.tool.smoothener.reset();
		...
		if (this.layoutBridge.liveMode) {
			...
			if (points.first.points) {
				...
				this.pathPart = Module.PathBuilder.createPath(
					chunk.toFloat32Array(),
					stride
				);
			} else {
				let pathPart;
				...
				points.forEach((point) => {
					pathPart = this.tool.pathBuilder.addPoint(
						this.inputPhase,
						point,
						point.pressure
					);
					chunk = chunk.concat(
						Module.readFloats(pathPart.points).toArray()
					);
				});

				pathPart = Module.PathBuilder.createPath(
					chunk.toFloat32Array(),
					pathPart.stride
				);
			...
			var pathPart = this.tool.pathBuilder.addPoint(
				this.inputPhase,
				point,
				point.pressure
			);
			var smoothedPathPart = this.tool.smoothener.smooth(
				pathPart,
				this.inputPhase == Module.InputPhase.End
			);
			var pathContext =
				this.tool.pathBuilder.addPathPart(smoothedPathPart);

			this.pathPart = pathContext.getPathPart();
			this.path = pathContext.getPath();

			if (this.inputPhase == Module.InputPhase.Move) {
				var preliminaryPathPart =
					this.tool.pathBuilder.createPreliminaryPath();
				var preliminarySmoothedPathPart = this.tool.smoothener.smooth(
					preliminaryPathPart,
					true
				);

				this.preliminaryPathPart =
					this.tool.pathBuilder.finishPreliminaryPath(
	...
	drawPath() {
		switch (this.inputPhase) {
			...
			case Module.InputPhase.End:
				...
				stroke.pointsRate = this.layoutBridge.liveMode
					? global.smartPad.pointsRate
					: 60;
	...
	refresh(dirtyArea) {
		...
		if (
			...
			this.strokesAndCurrentStrokeLayer.blend(this.strokesLayer, {
				mode: Module.BlendMode.NONE,
				rect: dirtyArea,
			});
			this.strokeRenderer.updatedArea = Module.RectTools.union(
				this.strokeRenderer.updatedArea,
				dirtyArea
			);

			if (
				!this.layoutBridge.liveMode &&
				this.inputPhase == Module.InputPhase.Move
			)
