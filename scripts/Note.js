...
class Note {
	constructor(data) {
		if (!data.locale) console.warn(`[Note: ${data.id}] Locale not found`);
		else if (!data.locale.contains("_"))
			console.warn(
				`[Note: ${data.id}] Invalid note locale found: ${data.locale}`
			);
		...
		this.lastModifiedDate =
			parseInt(data.lastModifiedDate) || this.creationDate;
		this.size = data.size;
		...
		} else this.transform = Module.MatTools.create();

		this.layers = data.layers || [];

		if (data.rawLayers) this.rawLayers = data.rawLayers;
	...
	hasImage(type) {
		return IOManager.existsSync(
			DBManager.getNotePaths(this.id).note +
				"/" +
				type +
				"_" +
				this.lastModifiedDate +
	...
	addLayer(data, atIndex) {
		...
		this.layers.splice(
			atIndex === undefined ? this.layers.length + 1 : atIndex,
			0,
			layer
		);
		return layer;
	...
	toJSON(layersData, discardLayers) {
		let data = {
			...
			stream: this.stream,
			tags: this.tags,
			layers: discardLayers
				? []
				: layersData || this.layers.map((layer) => layer.toJSON()),
		};
	...
	static fromJSON(brush, data) {
		...
		if (data.layers)
			noteData.layers = data.layers.map((layerData) =>
				Layer.fromJSON(brush, layerData)
			);
	...
	toWiddJSON(locale) {
		...
		this.layers.map((layer) => {
			let protoBuf = Buffer.from(
				Module.InkEncoder.encode(layer.strokes).buffer
			).toString("base64");

			inkArray.push(protoBuf);
		});

		let obj = {
			locale: newLocale,
			transform: `matrix(${this.transform.a} ${this.transform.b} ${this.transform.c} ${this.transform.d} ${this.transform.tx} ${this.transform.ty})`,
			boundingBox: this.isLandscape()
...
class Layer {
	...
	toJSON() {
		return {
			id: this.id,
			lastModifiedDate: this.lastModifiedDate,
			strokes: StrokesCodec.encode(this.strokes),
		};
	}

	static fromJSON(brush, data) {
		return new global.notesContext["Layer"](
			Object.assign({}, data, {
				strokes: StrokesCodec.decode(brush, data.strokes),
			})
		);
	}
}
...
class Entity {
	constructor(type, values = []) {
		...
		Object.defineProperty(this, "textValues", {
			get: function () {
				return Object.values(this.values).map((value) => value.name);
			},
		});
		Object.defineProperty(this, "orderedValues", {
			get: function () {
	...
	getID(name) {
		let items = Object.values(this.values).filter(
			(value) => value.name == name
		);

		if (items.length) {
			if (items.length == 1) return items.first.id;
			else
				throw new Error(
					"Name duplicates found: " + items.map((value) => value.id)
				);
		} else return null;
	}
	...
	cleanRelations(notes) {
		...
		Object.values(this.values).forEach((value) => {
			let removed = value.notes.filter(
				(noteID) => !notes.includes(noteID)
			);
			if (removed.length > 0) result[value.id] = removed;
		});

		Object.keys(result).forEach((valueID) =>
			this.removeRelation(valueID, ...result[valueID])
		);
	...
	toJSON() {
		return {
			type: this.type,
			values: Object.values(this.values).map((value) => value.toJSON()),
		};
	}

	static fromJSON(data) {
		return new global.notesContext["Entity"](
			data.type,
			data.values.map((value) => EntityValue.fromJSON(value))
		);
	}
}
...
class EntityValue {
	constructor(data) {
		...
		Object.defineProperty(this, "type", {
			get: function () {
				if (!this.entity)
					console.warn(
						`[EntityValue] ${this.name} has no entity relation`
					);
	...
	editRelation(...notes) {
		notes.forEach((noteID) => {
			if (this.notes.includes(noteID)) this.notes.remove(noteID);
			else this.notes.push(noteID);
		});
	}
