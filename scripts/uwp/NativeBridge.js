...
function resolveJSValue(obj) {
	if (obj.value) {
		if (obj.type == "JSON") return JSON.parse(obj.value);
		else if (obj.type == "ARRAY") return Array.from(obj.value);
		else if (obj.type == "JSON_ARRAY")
			return Array.from(obj.value).map((value) => JSON.parse(value));
		else if (obj.type == "BINARY") return Buffer.from(obj.value, "base64");
...
let UIManager = {
	...
	showSaveDialog: function (title, fileName, callback) {
		NativeUIManager.onsavedialogcomplete = (event) =>
			callback(event.detail[0].filePath);
	...
	showOpenDialog: function (
		...
		NativeUIManager.onopendialogcomplete = (event) =>
			callback(event.detail[0].filePaths);
		NativeUIManager.showOpenDialog();
	},
...
let IOManager = {
	...
	writeFile: function (filePath, data, callback) {
		...
		NativeIOManager.onwritefileerror = (event) =>
			callback(new Error(event.detail[0].error.message));
		NativeIOManager.onwritefileerror = (event) =>
			callback(
				resolveJSValue({ type: "ERROR", value: event.detail[0].error })
			);
	...
	writeFiles: function (filePath, data, extension, callback) {
		NativeIOManager.onwritefilescomplete = (event) => callback();
		NativeIOManager.onwritefileserror = (event) =>
			callback(new Error(event.detail[0].error.message));
		NativeIOManager.onwritefileserror = (event) =>
			callback(
				resolveJSValue({ type: "ERROR", value: event.detail[0].error })
	...
	readFile: function (filePath, encoding, callback) {
		NativeIOManager.onreadfilecomplete = (event) =>
			callback(event.detail[0].error, event.detail[0].data);
		NativeIOManager.onreadfileerror = (event) =>
			callback(new Error(event.detail[0].error.message));
		NativeIOManager.onreadfileerror = (event) =>
			callback(
				resolveJSValue({ type: "ERROR", value: event.detail[0].error })
		...
		NativeIOManager.readFile(filePath, encoding, callback);
	},
};
...
let NativeLinker = {
	...
	syncWithCloud: function (message) {
		if (message) NativeNativeLinker.syncWithCloud(message.accessToken);
		else NativeNativeLinker.syncWithCloud(null);
	},
...
NativeNativeLinker.onlog = (event) => {
	...
	else if (debug) console.info(args.message);
};
...
class UATracker {
	constructor(trackingID, userID) {
		this.origin = NativeTracker.createTracker(
			trackingID,
			userID,
			navigator.userAgent
		);
	...
	event(category, action, label, value) {
		...
		else this.origin.customEventWithValue(category, action, label, value);
