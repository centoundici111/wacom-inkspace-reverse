...
function sync(state, complete) {
	...
	if (state) syncTime = Date.now();
		...
		if (missingLayersNotes.length) {
			console.info(
				"===================== NOTES WITH NO LAYERS (" +
					missingLayersNotes.length +
					") ====================="
			);
...
class DBManager {
	...
	constructor(root) {
		...
		this.getNotePaths = DBBridge.prototype.getNotePaths;

		consoleBridge.init();

		this.glWorker = new ThreadBridge(
			global.ROOT + "/scripts/workers/GLWorker.js"
		);
	...
	async edit(entity, data, callback) {
		...
		await this.dbQueue.then(async () => {
			...
			for (let name in data) {
				if (data[name] == "REMOVE") delete entry[name];
				else entry[name] = data[name];
			}
	...
	getNotes(ids, serializeOutput, cloud) {
		...
		return this.notesQueue
			...
			.then((documents) => {
				if (ids.length)
					return documents.filter(
						(doc) =>
							ids.indexOf(
								CloudUtils.bufferToUuid(doc.payload.content)
							) != -1
					);
			...
			.then((documents) => {
				return Promise.all(
					documents.map((document) => {
						return cloudStorage
							.getSequence(document.payload)
							.then((pages) => {
								let noteID = CloudUtils.bufferToUuid(
								...
								if (pages.length) {
									...
									if (!stream || stream == "mate")
										return CloudNote.fromCloudNodes(
											document,
											pages[0]
										);
									else {
									...
									if (cloud) missingPagesNotes.push(noteID);
									else if (debug)
										console.warn(
											new Error(
												`[CLOUD] note with no pages detected: ${noteID}`
											)
										);
			...
			.then((notes) =>
				serializeOutput ? notes.map((note) => note.toJSON()) : notes
			);
	}

	getNote(id) {
		return this.getNotes([id]).then((notes) =>
			this.getPageLayers(notes[0].pageId).then((layers) =>
				notes[0].toJSON(layers)
			)
		);
	}
	...
	editNote(note, options, callback) {
		...
		else return this.editNotes([note], options);
	}
		...
		return this.notesQueue.then(() => {
			return new Promise((resolve, reject) => {
				this.exportNotes(notes, options, (notesLayers) => {
					...
					cloudStorage
						.getSequence(cloudStorage.root)
						.then((documents) => {
							notes.forEach((note) => {
								note.cloudNodes = note.toCloudNodes();
								let { documentNode } = note.cloudNodes;

								let docIndex = documents.findIndex((node) =>
									node.samePayload(documentNode)
								);
							...
							return cloudStorage.setSequence(
								cloudStorage.root,
								documents
							);
						})
						.then(() => {
							let input = notes.map((note) => {
								let { documentNode, pageNode } =
									note.cloudNodes;
								return () =>
									this.editNodes(documentNode.payload, [
										pageNode,
									]);
							});
						...
						.then(() => {
							let input = [].concat.apply(
								[],
								notes
									.filter((note) => note.hasLayers())
									.map((note) => {
										let { pageNode } = note.cloudNodes;
										let layerNodes = note.layers.map(
											(layer) => layer.toCloudNode()
										);

										return [[pageNode.payload, layerNodes]]
											.concat(
												note.layers.map((layer) => [
													layer.getLayerPayload(),
													layer.strokesToCloudNodes(),
												])
											)
											.map((item) => {
												return () =>
						...
						.then(() => {
							storeComm.send({
								action: "UPDATE_NOTES",
								body: notes.map((note) => note.toJSON([])),
							});
							resolve();
						})
						.catch((e) => {
		...
		return cloudStorage
			...
			.then((documents) => {
				notes.forEach((note) => {
					...
					if (docIndex == -1)
						Promise.reject(
							new Error(`Document ${note.id} not found`)
						);
					else {
			...
			.then(() => {
				let input = notes.map((note) => {
					...
					return () =>
						this.editNodes(documentNode.payload, [pageNode]);
				});
	...
	editNodes(payload, nodes) {
		return cloudStorage.getSequence(payload).then((prevNodes) => {
			nodes.forEach((node) => {
				let idx = prevNodes.findIndex((prevNode) =>
					prevNode.samePayload(node)
				);
	...
	exportNotes(notes, options, callback) {
		this.glWorker.send(
			"EXPORT_NOTES",
			{ input: { notes: notes, options: options } },
			callback
		);
	}
	...
	getPageLayers(pageID, serializeOutput) {
		return cloudStorage
			.getSequence(Payload.createUnique(CloudUtils.uuidToBuffer(pageID)))
			.then((layerNodes) => {
				return Promise.all(
					layerNodes.map((layerNode) => {
						...
						return cloudStorage
							.getSequence(layerNode.payload)
							.then((strokeNodes) => {
								layer.strokes =
									CloudLayer.strokesFromCloudNodes(
										strokeNodes
									);
	...
	deleteNote(id, callback) {
		...
		else return this.deleteNotes([id]);
	}
	...
	deleteNotes(ids) {
		return this.notesQueue
			...
			.then((documents) => {
				ids.map((id) =>
					Payload.createUnique(CloudUtils.uuidToBuffer(id))
				).forEach((payload) => {
					let docIndex = documents.findIndex((doc) =>
						doc.payload.equals(payload)
					);
	...
	getEntity(type) {
		...
		return cloudStorage
			.getSequence(reservedPayloads.globals)
			.then((entities) => {
				const entity =
					entities[
						entities.findIndex((entity) =>
							entity.payload.equals(payload)
						)
					];

				if (entity) return cloudStorage.getSequence(payload);
				else return Promise.resolve([]);
			})
			.then((values) => {
				return CloudEntity.fromCloudNode(
					type,
					Array.from(values)
				).toJSON();
			});
		...
		return cloudStorage
			...
			.then((entities) => {
				const idx = entities.findIndex((item) =>
					item.payload.equals(payload)
				);

				if (idx == -1) entities.push(entity.toCloudNode());
				else entities[idx] = entity.toCloudNode();

				return cloudStorage.setSequence(
					reservedPayloads.globals,
					entities
				);
			})
			.then(() => {
	...
	async initCloudStorage() {
		...
		cloudStorage = new CloudStorage(this.db.conn);
		...
		cloudStorage.on("CloudSyncSequenceBegin", (event) => {
			if (debug) {
				let key = event.parent.equals(cloudStorage.root)
					? event.parent.encodedData
					: CloudUtils.bufferToUuid(event.parent.content);
				console.log(
					`[CLOUD_SYNCING] SYNC SEQUENCE BEGIN (${key}, ${event.update})`
				);
			}
		});
		...
		cloudStorage.on("CloudSyncSequenceFinished", (event) => {
			if (debug)
				console.log(
					`[CLOUD_SYNCING] SYNC SEQUENCE FINISHED (${event.changes.length})`
				);
			...
			if (event.parent.equals(cloudStorage.root)) {
				let change = {
					update: event.changes
						...
						.map((change) =>
							CloudUtils.bufferToUuid(change.payload.content)
						),
					remove: event.changes
						.filter((change) => change.isDelete())
						.map((change) =>
							CloudUtils.bufferToUuid(change.payload.content)
						),
				...
				cloudQueue
					.then(this.get.bind(this, DBManager.entities.QUEUE))
					.then((queue) => {
						if (!queue) queue = [];
						queue.push(change);
				...
				let change = {
					update: event.changes
						.filter((change) => !change.isDelete())
						.map((change) => change.payload.content.toString()),
					remove: event.changes
						.filter((change) => change.isDelete())
						.map((change) => change.payload.content.toString()),
				};

				if (!globalsChange) globalsChange = change;
				else {
					globalsChange.update = globalsChange.update.concat(
						change.update.filter(
							(change) => !globalsChange.update.includes(change)
						)
					);
					globalsChange.remove = globalsChange.remove.concat(
						change.remove.filter(
							(change) => !globalsChange.remove.includes(change)
						)
					);
				}
				...
				if (!globalsChange) globalsChange = change;
					...
					!globalsChange.update.includes(
						groupsPayload.content.toString()
					)
				)
		...
		cloudStorage.on("CloudSyncFinished", (event) => {
			if (debug)
				console.log(
					`[CLOUD_SYNCING] SYNC FINISHED (${event.successful}, ${exporting})`
				);
				...
				globalsChange.update.forEach((type) => {
					cloudQueue
						.then(
							this.getEntity.bind(
								this,
								type.substring(2, type.length - 2)
							)
						)
						.then((entity) => {
							storeComm.send({
			...
			cloudQueue
				...
				.then((queue) => {
					...
					if (change.remove.length > 0) {
						...
						if (change.update.length == 0) queue.shift();
						else change.remove = [];
				...
				.then((notes) => {
					return Promise.all(
						notes.map((note) => this.getPageLayers(note.pageId))
					).then((layers) => ({ notes, layers }));
				})
				.then((result) => {
					let { notes, layers } = result;
					let notesData = notes.map((note, index) =>
						note.toJSON(layers[index])
					);
					...
					let complete = (reason) => {
						...
						if (synchronizing) synchronizeCallback = callback;
						else callback();
					};

					let updateNotes = utils.synchronize(
						(update) => {
							update = isDatabaseClean ? [] : update;
					...
					let next = () => {
						if (!syncing)
							return Promise.reject(
								"Syncing is canceled. User access is not found."
							);
						...
						if (noteData) {
							...
							if (hasLayers) {
								noteData.layers.forEach((layer) => {
									if (layer.strokes.length) hasStrokes = true;
									else hasEmptyLayer = true;
								});

								if (!hasStrokes) {
									console.warn(
										new Error(
											`[CLOUD] note with missing strokes detected: ${noteData.id}`
										)
									);
									// missingLayersNotes.push(noteData.id);
									missingLayersNotes.push(
										`${noteData.id} (${noteData.pageId})`
									);
								}
							} else {
								console.warn(
									new Error(
										`[CLOUD] note with missing layers detected: ${noteData.id}`
									)
								);
								// missingLayersNotes.push(noteData.id);
								missingLayersNotes.push(
							...
							if (!hasLayers) {
								cloudQueue
									.then(
										this.get.bind(
											this,
											DBManager.entities.QUEUE
										)
									)
									.then((queue) => {
										if (!queue)
											return Promise.reject(
												"Cloud queue is not found"
											);

										let change = queue[0];
										change.update.splice(
											change.update.indexOf(noteData.id),
											1
										);
								...
								if (hasEmptyLayer) {
									console.warn(
										new Error(
											`[CLOUD] note with empty layer detected: ${noteData.id}`
										)
									);
									console.info(
										noteData.id,
								...
								if (debug)
									console.log(
										`[CLOUD_EXPORTING] exporting ${noteData.id} started`
									);

								this.exportNotes([noteData], {}, () => {
									if (debug)
										console.log(
											`[CLOUD_EXPORTING] exporting ${noteData.id} completed`
										);
	...
	async migrateUnsynced() {
		...
		let buffer = await this.db.conn.get(bufferUnsynced).catch((reason) => {
			if (reason.type != "NotFoundError") throw reason;
		});

		if (!buffer || !buffer.length) return;

		const data = EncoderDecoder.readPayloadSequence(buffer).map(
			(payload) => payload.encodedData
		);
