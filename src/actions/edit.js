...
import { Layer, Note } from "../../scripts/Note";
...
function updateLayer(strokes, initialDate) {
	...
	strokes.forEach((stroke) => {
		...
		if (!stroke.timestamp) {
			...
			creationDate += Math.ceil(
				(stroke.length / stroke.pointsRate) * 1000
			);
	...
	return (dispatch, getState) => {
		...
		if (newNote) {
			...
			if (DeviceManager.device) {
				...
				if ("orientation" in DeviceManager.device)
					orientation = DeviceManager.device.orientation;
			}
			...
			note.onTouch = () =>
				dispatch({ type: ActionTypes.EDIT_NOTE_UPDATED });
				...
				DBManager.getPageLayers(note.pageId)
					...
					.then(() => {
						if (note.layers.length) {
							note.onTouch = () =>
								dispatch({
									type: ActionTypes.EDIT_NOTE_UPDATED,
								});

							dispatch({
								type: ActionTypes.EDIT_MODE_INIT,
								body: note,
							});
							dispatch(closeDialog());

							UAManager.edit("Edit Content", "Open Canvas");
						} else dispatch(push("/library"));
					});
			} else dispatch(push("/library"));
		}
	};
	...
	return (dispatch, getState) => {
		...
		if (!saving) {
			...
			DBManager.editNotes([note])
				...
				.then(() => {
					dispatch({
						type: ActionTypes.EDIT_END_SAVE,
						body: note.id,
					});
...
function triggerSaveNote() {
	return (dispatch, getState) => {
		...
		if (note.lastModifiedDate != lastModifiedDate)
			dispatch(openDialog(Modals.SAVE_EDIT_CHANGES));
		else if (!saving) AppManager.confirmSaveNote();
	};
}
...
function removeStrokesFromCurrentLayer(strokes) {
	return (dispatch, getState) => {
		...
		strokes.forEach((stroke) =>
			note.layers[currentLayer].strokes.remove(stroke)
		);
...
function initNoteSplit() {
	...
	return (dispatch, getState) => {
		...
		dispatch({
			type: ActionTypes.EDIT_INIT_SPLIT,
			body: { splitMode: "note" },
		});
	};
}
...
function initLayerSplit(targetLayer) {
	...
	return (dispatch, getState) => {
		...
		dispatch({
			type: ActionTypes.EDIT_INIT_SPLIT,
			body: { splitMode: "layer", targetLayer },
		});
	};
}
...
function cancelSplit() {
	return (dispatch, getState) => {
		...
		UAManager.edit(
			"Split Page",
			toCamelCase(splitMode) + " Split Canceled"
		);
	};
}
	...
	return (dispatch, getState) => {
		...
		if (splitMode == "layer") {
			...
			if (splitIndex > 0 && splitIndex < splitIndexMax) {
				let newLayer = new Layer({
					strokes:
						note.layers[currentLayer].strokes.slice(splitIndex),
				});
				note.layers[currentLayer].strokes = note.layers[
					currentLayer
				].strokes.slice(0, splitIndex);
				...
				queue
					.then(() => {
						...
						dispatch({
							type: ActionTypes.EDIT_UPDATE_PREVIEWS,
							body: previews,
						});
					})
					.catch(console.error);
		...
		if (splitMode == "note" && !applyingSplitNote) {
			if (splitIndex > 0 && splitIndex < splitIndexMax) {
				...
				for (
					...
					if (layerSplitIndex < strokes.length) {
						if (layerSplitIndex == 0) {
							...
							newNote.layers = note.layers.slice(layerIndex);
							newNote.layers[0] = new Layer({
								strokes:
									newNote.layers[0].strokes.slice(
										layerSplitIndex
									),
							});

							note.layers = note.layers.slice(0, layerIndex + 1);
							note.layers[note.layers.length - 1].strokes =
								note.layers[
									note.layers.length - 1
								].strokes.slice(0, layerSplitIndex);
				...
				DBManager.editNotes([note, newNote])
					.then(() => {
						let noteGroups = ContentManager.getEntityRelations(
							"groups",
							noteID
						);
					...
					.then(() => {
						...
						dispatch(
							addNotification("notification.edit.noteSplit")
						);
						...
						dispatch(
							addNotification("notification.edit.noteSaved")
						);
...
function layerAdd() {
	return (dispatch, getState) => {
		...
		if (note.isLandscape()) previews.push(EMPTY_LANDSCAPE_THUMB);
		else previews.push(EMPTY_PORTRAIT_THUMB);
...
function layerCopy(targetLayer) {
	...
	return (dispatch, getState) => {
		...
		let clipboard = {
			...
			meta: layer.map((stroke) => ({
				timestamp: stroke.timestamp,
				pointsRate: stroke.pointsRate,
			})),
		};
...
function layerPaste(targetLayer) {
	...
	return (dispatch, getState) => {
		...
		if (currentLayer >= targetLayer) currentLayer += 1;
		...
		if (!Object.equals(note.transform, clipboard.transform)) {
			...
			transform = Module.MatTools.multiply(
				transform,
				clipboard.transform
			);
...
function layerMergePrevious(targetLayer) {
	...
	return (dispatch, getState) => {
		...
		note.layers[targetLayer - 1].strokes = [
			...note.layers[targetLayer - 1].strokes,
			...note.layers[targetLayer].strokes,
		];
		note.layers.splice(targetLayer, 1);
