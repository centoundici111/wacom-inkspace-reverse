	...
	switch (action.type) {
		case ActionTypes.ADD_NOTIFICATION: {
			...
			if (action.body.translatable) {
				if (LocalesManager.dictionary[msg]) {
					...
					Object.keys(action.body.props).forEach(
						(name) =>
							(msg = msg.replace(
								"{" + name + "}",
								action.body.props[name]
							))
					);
				} else console.warn("translation missing:", action.body);
			}
			...
			if (state.notifications.isEmpty())
				...
				(notificationsQueue.isEmpty() ||
					(!notificationsQueue.isEmpty() &&
						notificationsQueue.last().message !=
		...
		case ActionTypes.REMOVE_NOTIFICATION: {
			...
			if (typeof action.body == "string")
				entry = state.notifications.find(
					(notification) => notification.key == action.body
				);
