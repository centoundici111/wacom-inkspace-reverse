...
class MainMenuManager extends MenuManager {
	...
	assignMenu(page, props) {
		...
		switch (page) {
			...
			default:
				throw new Error(
					`Menu page ${page} not found. Failed to init menu.`
				);
		}
	...
	getInitialMenu(templates) {
		let menu = new Menu();
		let fteTemplate =
			process.platform == "darwin" ? templates.fte : templates.fteFile;
	...
	getLibraryMenu(templates) {
		...
		if (process.platform == "darwin") {
			...
			else menu.append(new MenuItem(templates.file));
	...
	getMenuTemplates(authorized) {
		...
		let saveEnabled = !!(
			editState.note &&
			editState.note.strokes.length > 0 &&
			editState.saveEnabled
		);
		...
		return {
			...
			groupsFile: {
				...
				submenu: {
					tag: new MenuItem({
						label: LocalesManager.dictionary[
							"tooltip.tags.manager"
						],
						enabled:
							appState.online &&
							ContentManager.selected.length > 0 &&
							!libraryState.rotateInProgress,
						click: () => {
							if (AuthenticationManager.hasAccess()) {
								this.store.dispatch(
									modalsAction.openDialog(Modals.TAGS_MANAGER)
								);
							} else {
								this.store.dispatch(
					...
					delete: {
						...
						click: () =>
							this.store.dispatch(
								modalsAction.openDialog(Modals.REMOVE_GROUP)
							),
					},
					separatorExit: {
						type: "separator",
			...
			file: {
				...
				submenu: {
					...
					rotate: {
						...
						click: () => {
							this.store.dispatch(
								libraryAction.rotateTransformAdd90deg()
							);
							setTimeout(
								() =>
									this.store.dispatch(
										libraryAction.rotateNotes(
					...
					delete: {
						...
						click: () => {
							if (ContentManager.selected.length > 0)
								this.store.dispatch(
									modalsAction.openDialog(Modals.CONFIRM, {
										title: "confirm.remove.note.title",
										content:
											"confirm.remove.note.description",
										type: "REMOVE_NOTE",
										noteIDs: ContentManager.selected,
									})
								);
						},
					},
			...
			fileEdit: {
				...
				submenu: {
					save: {
						...
						click: () =>
							this.store.dispatch(editActions.saveNote()),
					},
					close: {
						label: LocalesManager.dictionary["system.menu.close"],
						click: () =>
							this.store.dispatch(
								editActions.transferToLibrary()
							),
					},
					separatorExit: {
						type: "separator",
			...
			fileLive: {
				...
				submenu: {
					close: {
						label: LocalesManager.dictionary["system.menu.close"],
						click: () =>
							this.store.dispatch(editActions.closeNote()),
					},
					separatorExit: {
						type: "separator",
						remove: process.platform == "darwin",
			...
			window: {
				label:
					process.platform == "darwin"
						? LocalesManager.dictionary["system.menu.window"]
						: LocalesManager.dictionary["system.menu.view"],
				submenu: {
					minimize: {
						label: LocalesManager.dictionary[
							"system.menu.minimize"
						],
						role: "minimize",
					},
					maximize: {
						label:
							process.platform == "darwin"
								? LocalesManager.dictionary["system.menu.zoom"]
								: LocalesManager.dictionary[
										"system.menu.maximize"
								  ],
			...
			edit: {
				...
				submenu: {
					splitNote: {
						...
						click: () =>
							this.store.dispatch(editActions.initNoteSplit()),
					},
					// undo: {
			...
			layer: {
				...
				submenu: {
					copy: {
						...
						click: () => {
							this.store.dispatch(
								editActions.layerCopy(editState.currentLayer)
							);
							this.store.dispatch(
								appAction.addNotification(
									"notification.edit.layerCopy"
								)
							);
						},
					},
					paste: {
						...
						click: () => {
							this.store.dispatch(
								editActions.layerPaste(editState.currentLayer)
							);
							this.store.dispatch(
								appAction.addNotification(
									"notification.edit.layerPaste"
								)
							);
						},
					},
					delete: {
						...
						enabled:
							!!editState.note &&
							editState.note.layers.length >= 2,
						click: () => {
							this.store.dispatch(
								editActions.layerDelete(editState.currentLayer)
							);
							this.store.dispatch(
								appAction.addNotification(
									"notification.edit.layerDelete"
								)
							);
						},
					},
					previous: {
						...
						click: () =>
							this.store.dispatch(
								editActions.layerMergePrevious(
									editState.currentLayer
								)
							),
					},
					split: {
				...
				submenu: [
					{
						...
						click: AppManager.forceReload.bind(AppManager),
					},
					{
						label: "Force Reload",
			...
			help: {
				...
				submenu: {
					help: {
						...
						click: () => {
							if (
								...
								UIManager.openExternal(
									"https://www.wacom.com/getting-started/wacom-intuos-pro-paper-edition"
								);
							else
								UIManager.openExternal(
									"https://www.wacom.com/getting-started/wacom-sketchpad-pro"
								);
						},
					},
					otherApps: {
						label: LocalesManager.dictionary[
							"system.menu.other.apps"
						],
						click: () =>
							UIManager.openExternal(
								"http://www.wacom.com/products/apps-services/bamboo-paper"
							),
					},
					accessories: {
						label: LocalesManager.dictionary[
							"system.menu.accessories"
						],
						click: () =>
							UIManager.openExternal(
								"https://estore.wacom.com/?utm_source=inkspace%20app%20-%20desktop&utm_medium=application&utm_content=accessories"
							),
					},
					checkForUpdate: {
						key: "checkForUpdate",
						label: LocalesManager.dictionary[
							"system.menu.check.for.updates"
						],
						click: () => {
					...
					checkingForUpdate: {
						...
						label: LocalesManager.dictionary[
							"system.menu.checking.for.update"
						],
					},
					restartToUpdate: {
						key: "restartToUpdate",
						label: LocalesManager.dictionary[
							"system.menu.restart.to.update"
						],
						click: () => {
	...
	*/
	getAboutTemplate(help) {
		...
		if (process.platform == "darwin") remove = !!help;
		...
		if (process.platform == "darwin") about.role = "about";
		else
			about.click = () =>
				this.store.dispatch(
					modalsAction.openSettings(SettingsTab.ABOUT)
				);
