...
let AppManager = {
	...
	init: function () {
		...
		addEventListener("resize", (e) => {
			...
			this.updateWindowSizeID = setTimeout(() => {
				DBManager.edit(DBManager.entities.SETTINGS, {
					window: {
						width:
							window.outerWidth < WINDOW_MIN_WIDTH
								? WINDOW_MIN_WIDTH
								: window.outerWidth,
		...
		window.onbeforeunload = (e) => {
			if (debug)
				console.log(
					"[APP MANAGER] onbeforeunload (reload: %s, closing: %s, closed: %s)",
					this.reload,
					this.closing,
					this.closed
				);
			...
			if (!this.closing) {
				...
				e.returnValue = false;
			} else if (!this.closed) e.returnValue = false;
			else if (!this.reload) UIManager.quit();
		};

		window.ondragover = window.ondrop = (e) => {
			e.preventDefault();
			e.stopPropagation();
		};
	},
	...
	closeSmartPad: function () {
		DeviceManager.close()
			.then((closing) => {
				if (!closing) AppManager.complete();
			})
			.catch((reason) => {
				if (reason.message) console.error(reason);
				else console.info(reason);
	...
	complete: function () {
		if (
			!this.editCompleted &&
			DeviceManager.context == DeviceManager.Context.LIVE
		) {
			this.triggerLiveSaveNote();
			return;
		} else if (
			!this.editCompleted &&
			DeviceManager.context == DeviceManager.Context.CREATION
		) {
			this.triggerEditSaveNote();
			return;
		}
		...
		setTimeout(function () {
			...
			if (AppManager.reload) UIManager.reload();
			else DBManager.closeDB().then(() => window.close());
		}, 500);
	},
};
