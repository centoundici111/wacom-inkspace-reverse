...
let UAManager = {
	...
	update: function () {
		...
		if (!uuid && this.uuid) UAManager.cloud("Log Out", "Log Out Confirmed");

		this.uuid = uuid;

		if (debug)
			console.log(
				"[UA_MANAGER] update identity:",
				uuid || this.appUserID
			);
		else {
			if (this.visitor) this.visitor.set("uid", uuid || this.appUserID);
			else {
				this.visitor = UATracker.createInstance(
					project.uaTrackingID,
					uuid || this.appUserID
				);
...
Object.getOwnPropertyNames(UAManager).forEach((name) => {
	UAManager[name] = () => {};
});
