...
let ContentManager = {
	open() {
		return DBManager.getNotes()
			.then((notes) => this.init(notes))
			.then(() => DBManager.getEntity("groups"))
			.then((groups) => this.setEntity(groups));
	},
	...
	updateSections(callback) {
		...
		if (this.searchData) {
			...
			if (this.filters["GROUP"])
				notes = notes.filter((note) =>
					this.getEntity("groups")
						.get(this.filters["GROUP"])
						.notes.includes(note.id)
				);
			if (this.filters["TAG"])
				notes = notes.filter((note) =>
					note.tags.includes(this.filters["TAG"])
				);
		}
		...
		for (let section of cache.values()) sections.push(section);

		let selected = this.selected.filter((selected) =>
			notes.some((note) => note.id == selected)
		);

		if (!selected.length && sections.length)
			selected = [sections.last.last];

		if (this.searchData) sections = [[].concat(...sections)];

		this.sections = sections;
		this.selected = selected;
		this.lastSingleSelected = selected.first;
	...
	edit(notes) {
		...
		notes.forEach((note) => {
			...
			if (
				this.notes[note.id] &&
				this.notes[note.id].creationDate != note.creationDate
			)
		...
		if (notesForAdd.length) this.updateSections();
		else this.onUpdate();
	},
	...
	remove(ids, skip) {
		...
		let sections = this.sections
			.map((section) => section.filter((id) => !ids.includes(id)))
		...
		if (!skip) this.onUpdate();
	},

	replace(notesForRemove, note) {
		if (debug)
			console.log(
				"[CONTENT_MANAGER] replace",
				notesForRemove.map((note) => note.id),
				" => ",
				note.id
			);
		...
		this.remove(
			notesForRemove
				.filter((noteForRemove) => noteForRemove.id != note.id)
				.map((note) => note.id),
			true
		);
	...
	getNearestNeighbour(idsForRemove) {
		...
		let result = selectedIDs.filter((noteID) => {
			for (let i = 0; i < idsForRemove.length; i++) {
				if (idsForRemove[i] == noteID) return false;
			}
		...
		for (let i = 0; i < noteIDs.length; i++) {
			if (noteIDs[i] == selectedID) {
				if (i == 0) nearestNeighbour = noteIDs[i + 1];
				else nearestNeighbour = noteIDs[i - 1];

				break;
			}
		}

		if (nearestNeighbour) result = [nearestNeighbour];

		return result;
	},

	filter(type, value) {
		if (!type || type.type != "FilterType")
			throw new Error("Unknown content filter found: " + type);
		if (debug) console.log("[CONTENT_MANAGER] filter", type.name, value);

		// TODO: fix value.id || value when TAGS refactoring
		if (value) this.filters[type.name] = value.id || value;
		else delete this.filters[type.name];
	...
	getEntity(type) {
		...
		// Fix for https://jira.wacom.com/browse/MDIW-1868
		if (type === "groups" && Object.keys(entity.values).length > 0) {
			let filteredObject = Object.keys(entity.values).reduce(function (
				r,
				e
			) {
				if (entity.values[e].stream === "mate") {
					r[e] = entity.values[e];
				}
	...
	getEntityNotes(type, valueID, sorted) {
		...
		if (valueID)
			result = entity.values[valueID].notes
				.map((noteID) => this.notes[noteID])
				.filter((note) => !!note);
		else {
			var set = new Set();

			Object.values(entity.values).forEach((value) => {
				for (let noteID of value.notes) set.add(noteID);
			});
	...
	getEntityPages(type, valueID, sorted) {
		return this.getEntityNotes(type, valueID, sorted).map(
			(note) => note.pageId
		);
	},
	...
	setEntity(entity) {
		...
		if (valueID) {
			if (entity.get(valueID)) this.updateSections();
			else this.filter(ContentManager.FilterType[filterType], null);
		} else this.onUpdate();
	},
		...
		AuthenticationManager.onSyncComplete(() => {
			...
			Object.values(this.entities).forEach((entity) => {
				...
				if (removed) {
					if (debug)
						console.log(
							`[CONTENT_MANAGER] cleanEntityRelations(${entity.type}):`,
							removed
						);
			...
			Promise.all(
				entitiesToUpdate.map((entity) => DBManager.setEntity(entity))
			);
		});
	},
		...
		if (data) {
			...
			data.forEach((node) => {
				let index = notes.findIndex(
					(note) => note.pageId == node.nodeId
				);
				let note = notes[index];

				if (note) searchData[note.id] = node.txt;
			});
	...
	select(ids) {
		if (
			...
			!ids.filter((id) => !this.selected.includes(id)).length
		)
			return;
		if (debug) console.log("[CONTENT_MANAGER] select", ids);

		if (ids.length === 1) {
			this.lastSingleSelected = ids[0];
		}
	...
	getLatestNote(ids) {
		return ids
			...
			.sort(comparator({ sortBy: "lastModifiedDate" })).first;
	},
...
Function.prototype.createEnum.call(ContentManager, "FilterType", [
	"GROUP",
	"TAG",
]);
