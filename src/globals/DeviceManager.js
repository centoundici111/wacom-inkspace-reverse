...
let DeviceManager = {
	...
	init(appID) {
		...
		Object.defineProperty(this, "device", {
			set: (value) => {
				...
				if (value && value != "REMOVE") device = value;
				else {
					if (value == "REMOVE") device = null;
					else delete device[device.protocol];
		...
		return DBManager.get(DBManager.entities.DEVICE)
			.then((value) => {
				device = value;
				...
				if (
					...
					typeof device.firmwareVersion.version == "string"
				)
					device.firmwareVersion = {
						version: [device.firmwareVersion.version],
						type: [[device.firmwareVersion.type]],
					};
			})
			.then(() => {
				...
				if (DeviceManager.BTCL) {
					if (global.SmartPadSPP) {
						...
						smartPadBTC = new SmartPadSPP(
							appID,
							::this.resolveAuthorize
						);
						smartPadBTC.inputTransformer =
							new DeviceInputTransformer();
				...
				return SmartPadUSB.attachListeners(
					(id, error) => {
						if (debug)
							console.log(
								"[DEVICE_MANAGER] usb device attached:",
								id,
								"found:",
								SmartPadUSB.devices.length
							);

						if (error) {
							if (smartPadUSB.debug) console.error(error);
							this.addNotification(
								"notification.malfunction.device.detected"
							);
						} else {
					...
					(id) => {
						if (debug)
							console.log(
								"[DEVICE_MANAGER] usb device detached:",
								id,
								"left:",
								SmartPadUSB.devices.length
							);
	...
	configure: function (context) {
		...
		if (
			!global.smartPad &&
			this.device &&
			context != DeviceManager.Context.SETUP
		) {
			...
			if (this.device.protocol == "USB" && this.device["USB"])
				...
				(this.device.protocol == "BTC" && this.device["BTC"]) ||
				(this.device.protocol == "SPP" && this.device["SPP"])
			)
		...
		switch (context) {
			...
			case DeviceManager.Context.LIBRARY:
				...
				if (exitSetup && global.smartPad && !smartPad.isOpen(true)) {
					...
					if (deviceInfo) smartPad.open(deviceInfo);
				}
	...
	open(protocol, deviceInfo, deviceType) {
		if (protocol == "DEFAULT") {
			if (global.smartPad) {
				...
				} else this.setDeviceStatus(DeviceStatus.DISCONNECTED);
		...
		if (protocol == "USB") {
			if (SmartPadUSB.devices.length > 0) {
				...
				else this.openDialog(Modals.NO_SUPPORTED_DEVICE);
			}
				...
				SmartPadSPP.stopScanner()
					...
					.catch(() => {
						...
						this.checkConnID = setTimeout(() => {
							if (debug)
								console.log(
									"[DEVICE_MANAGER] check SPP connected"
								);
			...
			if (deviceInfo) this.smartPad.open(deviceInfo);
			else {
				clearInterval(this.checkConnID);

				this.reconfigure(() => {
					this.smartPad =
						this.type == "VIPER" ? smartPadBTC : smartPadBLE;
					this.smartPad.open();
				});
			}
		} else throw new Error("Unknown protocol detected:", protocol);
	},
	...
	setName: function (name) {
		...
		if (
			smartPad.isOpen() &&
			(smartPad.state == SmartPadNS.State.READY || !this.downloading)
		)
	...
	rename: function (name) {
		return new Promise((resolve, reject) => {
			if (name == this.device.name) resolve();
			else {
				smartPad
					.switchToReadyMode()
					.then(() => {
					...
					.then(() => {
						...
						DBManager.edit(DBManager.entities.DEVICE, {
							name: name,
						});
						...
						else return smartPad.switchToFileTransferMode();
					})
					.catch(reject);
			}
		});
	...
	close: function () {
		return new Promise((resolve, reject) => {
			if (global.smartPad) {
				if (smartPad.isOpen()) {
					if (
						smartPad instanceof SmartPadUSB &&
						smartPad.state == SmartPadNS.State.REAL_TIME
					) {
						if (smartPad.isReady()) {
							smartPad
								.switchToReadyMode()
								...
								.catch(reject);
						} else reject("smartPad is busy, waiting...");
					} else {
						resolve(true);
						smartPad.close(true);
					}
	...
	triggerLiveMode: function () {
		if (!global.smartPad) {
			...
			this.addNotification(
				"notification.no.live.mode.while.note.transfer"
			);
			return;
		}
		...
		if (smartPad.isReady()) {
			smartPad
				...
				.catch((reason) => {
					...
					this.addNotification(
						"notification.trigger.live.mode.failed"
					);
				});
		} else {
			...
			this.openDialog(
				"LiveMode" +
					(smartPad.protocol == "SPP" ? "BTC" : smartPad.protocol) +
					"Waiting"
			);
	...
	reconfigure: function (callback) {
		this.completeReconfigure = (timeout) => {
			...
			if (timeout) setTimeout(callback, 250);
			else callback();
		};

		if (global.smartPad && global.smartPad.isOpen(true))
			smartPad.close(true);
		else this.completeReconfigure();
	},
	...
	updateOrientation: function (orientation, edit) {
		...
		if (this.device) this.device.orientation = orientation;
	...
	reconnect: function (instant) {
		...
		if (
			!global.smartPad ||
			!this.device ||
			(!this.device["USB"] && !this.device["BTC"]) ||
			smartPad.transportProtocol == SmartPadNS.TransportProtocol.BLE
		)
			return;

		if (smartPad.isOpen() || this.discardReconnect) return;

		if (this.context == DeviceManager.Context.SETUP || this.forgetDevice) {
			clearTimeout(this.reconnectTimeoutID);
			this.reconnectTimeoutID = setTimeout(
				() => this.reconnect(),
				DeviceManager.RECONNECT_TIMEOUT
			);
		...
		if (debug && !instant)
			console.log(
				"[DEVICE_MANAGER] attempt to reconnect",
				this.context.name
			);
			...
			return new Promise((resolve, reject) => {
				if (smartPad && smartPad.deviceInfo) {
					...
					if (smartPad instanceof SmartPadUSB) {
						if (
							SmartPadUSB.devices.includes(smartPad.deviceInfo.id)
						)
							resolve();
						else {
							let error;
						...
						this.serialPortSupport &&
						smartPad instanceof SmartPadSPP
					)
						SmartPadSPP.list(smartPad.deviceInfo)
							.then((devices) => resolve(smartPad.deviceInfo))
							.catch(reject);
					else {
						SmartPadBTC.list(false)
							.then((devices) => {
								if (
									devices
										.map((device) => device.id)
										.includes(smartPad.deviceInfo.id)
								)
									resolve();
								else reject();
							})
		...
		let resolveLookup = (smartPad, deviceInfo) => {
			deviceInfo = Object.assign({}, deviceInfo || smartPad.deviceInfo, {
				reconnect: true,
			});
			...
			if (
				this.serialPortSupport &&
				smartPad.transportProtocol == SmartPadNS.TransportProtocol.USB
			)
				SmartPadSPP.stopScanner()
					.then(() => smartPad.open(deviceInfo))
					.catch(console.error);
			else smartPad.open(deviceInfo);
		};

		clearTimeout(this.reconnectTimeoutID);

		if (instant) protocolsLookup([smartPadUSB, smartPadBTC]);
		else
			this.reconnectTimeoutID = setTimeout(
				() => this.reconnect(true),
				DeviceManager.RECONNECT_TIMEOUT
	...
	initContextEvents: function () {
		this.liveStatusUpdate = (event) => {
			if (
				event.status == SmartPadNS.Status.CLOSED ||
				event.status == SmartPadNS.Status.DISCONNECTED
			) {
				WILL.abort();

				if (this.liveDeviceClosed) this.liveDeviceClosed();
			}
		};

		setInterval(
			DeviceManager.download.bind(DeviceManager),
			DeviceManager.DOWNLOAD_INTERVAL
		);
	},
		...
		smartPad.on("StatusUpdate", (event) => {
			...
			switch (event.status) {
				...
				case SmartPadNS.Status.CLOSED:
					...
					if (this.downloading) this.completeDownload();
				...
				case SmartPadNS.Status.AUTHORIZE_EXPECT:
					if (this.context == DeviceManager.Context.SETUP) {
						...
						this.addNotification(
							"notification.device.restore.connection"
						);
				...
				case SmartPadNS.Status.AUTHORIZE_FAILED:
					switch (event.authorizeFailType) {
						case SmartPadNS.AuthorizeFailType.FOREIGN_DEVICE:
							if (this.context == DeviceManager.Context.SETUP) {
								if (smartPad.protocol == "SPP")
									this.closeDialog();
								this.openDialog(Modals.CONNECTION_LOST);
							} else {
								this.device = null;
								DeviceManager.addNotification(
									"notification.device.not.recognized"
								);
							}
	...
	resolveAuthorize() {
		return new Promise((resolve, reject) => {
			if (
				this.context == DeviceManager.Context.SETUP ||
				smartPad.transportProtocol == SmartPadNS.TransportProtocol.BTC
			)
				resolve();
			else {
				let timeoutID;
				...
				this.onCloseForgetDevice = (forget) => {
					...
					if (forget) reject("FORGET_DEVICE");
					else {
	...
	updateDeviceInfo() {
		let complete = (batteryState) => {
			if (this.context == DeviceManager.Context.SETUP) {
				...
				} else this.moveWizardTo(WizardSteps.SET_NAME);
			}

			return Promise.resolve()
				.then(() => {
					if (!batteryState) return smartPad.getBatteryState();
					else return Promise.resolve(batteryState);
				})
				.then((batteryState) => {
					this.setBatteryCharge(batteryState);

					if (this.onDeviceReady) return this.onDeviceReady();
		...
		if (
			this.device &&
			this.device.protocol == "BLE" &&
			this.device["BLE"] &&
			smartPad.transportProtocol == SmartPadNS.TransportProtocol.BLE &&
			smartPad.serial == this.device.id
		) {
		...
		smartPad
			...
			.then((batteryState) => {
				if ("orientation" in smartPad)
					device.orientation = smartPad.orientation;
			...
			.catch((reason) => {
				...
				if (smartPad.isOpen())
					setTimeout(
						() => this.updateDeviceInfo(),
						DeviceManager.UPDATE_DEVICE_RETRY_TIMEOUT
					);
			});
	},
	...
	navigateToBegining: function () {
		...
		else this.moveWizardTo(WizardSteps.SWITCH_ON);
	},
	...
	keepAlive: function () {
		if (
			...
			DeviceManager.keepAliveTimeoutID = setTimeout(
				() =>
					smartPad
						.getDataSessionState()
						.then(() => DeviceManager.keepAlive())
						.catch(console.error),
				DeviceManager.KEEP_ALIVE_TIMEOUT
			);
		}
	},
	...
	attachLiveEvents: function (smartPad) {
		...
		smartPad.on("ResetRealtimeDataBuffer", () => {
			...
			if (this.liveNewPage) this.liveNewPage();
		});

		smartPad.on("NewLayer", () => {
			WILL.abort();

			if (this.liveNewLayer) this.liveNewLayer();
		});

		smartPad.on("StrokeStart", (event) => {
			if (this.liveStrokeStart)
				this.liveStrokeStart(
					event.timestamp,
					event.penType,
					event.penID
				);
		...
		smartPad.on("PointReceived", (event) => {
			if (this.livePointReceived)
				this.livePointReceived(
					event.phase,
					event.point || event.pathPart,
					event.path
				);
	...
	download: function () {
		...
		if (!this.context || this.context == DeviceManager.Context.SETUP)
			return;
		...
		let next = () => {
			smartPad
				...
				.then((fileInfo) => {
					if (debug)
						console.log(
							"[DOWNLOAD] File Info - created:",
							new Date(fileInfo.timestamp),
							"size:",
							fileInfo.size
						);
...
Function.prototype.createEnum.call(DeviceManager, "Context", [
	"SETUP",
	"LIBRARY",
	"CREATION",
	"LIVE",
