...
class ContextMenuManager extends MenuManager {
	...
	init() {
		this.groupMenu = this.buildMenu([
			{
				...
				click: () => {
					...
					this.store.dispatch(
						modalsAction.openDialog(Modals.ENTITY, {
							type: "groups",
							label: "entity.group.name",
							value: ContentManager.getEntity("groups").get(
		...
		this.languagesMenu = this.buildMenu(
			LocalesManager.displayLocales.map((locale) => ({
				value: locale,
				label: LocalesManager.getLangName(locale),
				click: () => this.setLanguage(locale),
			}))
		);
		this.exportLanguagesMenu = this.buildMenu(
			LocalesManager.noteLocales.map((locale) => ({
				value: locale,
				label: LocalesManager.getLangName(locale),
				click: () => this.setExportLanguage(locale),
			}))
		);
		...
		this.lassoContextMenu = this.buildMenu([
			{ label: LocalesManager.dictionary["contextmenu.copy"] },
			{
				label: LocalesManager.dictionary[
					"contextmenu.lasso.duplicate.page"
				],
			},
	...
	setExportLanguage(noteLocale) {
		...
		if (this.exportUpdateDefault) {
			...
			UAManager.settings(
				"Set Note Language",
				LocalesManager.getLangName(noteLocale)
			);
		...
		let template = {
			copy: {
				...
				click: () => {
					...
					this.store.dispatch(
						appAction.addNotification("notification.edit.layerCopy")
					);
				},
			},
			paste: {
				...
				click: () => {
					this.store.dispatch(
						editAction.layerPaste(this.targetLayer)
					);
					this.store.dispatch(
			...
			delete: {
				...
				click: () => {
					this.store.dispatch(
						editAction.layerDelete(this.targetLayer)
					);
					this.store.dispatch(
						appAction.addNotification(
			...
			merge: {
				...
				click: () => {
					this.store.dispatch(
						editAction.layerMergePrevious(this.targetLayer)
					);
				},
			},
			split: {
				...
				click: () => {
					this.store.dispatch(
						editAction.initLayerSplit(this.targetLayer)
					);
				},
			},
		};
	...
	showNoteMenu() {
		...
		let template = {
			...
			combine: {
				...
				click: () => {
					if (ContentManager.selected.length > 1)
						this.store.dispatch(
							libraryAction.combineNotes(ContentManager.selected)
						);
				},
			},
			tag: {
				...
				click: () => {
					if (AuthenticationManager.hasAccess())
						...
						this.store.dispatch(
							modalsAction.openDialog(Modals.WACOM_ID_BENEFITS)
						);
				},
			},
			...
			select: {
				label:
					ContentManager.selected.length ===
						[].concat(...ContentManager.sections).length &&
					[].concat(...ContentManager.sections).length !== 1
						? LocalesManager.dictionary["menu.deselect.all"]
						: LocalesManager.dictionary["menu.select.all"],
				enabled: [].concat(...ContentManager.sections).length !== 1,
				click: () => {
					if (
						ContentManager.selected.length ===
						[].concat(...ContentManager.sections).length
						...
						this.store.dispatch(
							libraryAction.selectNotes(
								ContentManager.lastSingleSelected
							)
						);
					} else {
						this.store.dispatch(
							libraryAction.selectNotes(
								[].concat(...ContentManager.sections)
							)
						);
					}
				},
					...
					if (ContentManager.selected.length > 0)
						this.store.dispatch(
							modalsAction.openDialog(Modals.CONFIRM, {
								...
								content: "confirm.remove.note.description",
								type: "REMOVE_NOTE",
