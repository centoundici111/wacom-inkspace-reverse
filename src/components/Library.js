...
class Library extends Component {
	...
	applyUI(delta, gridWidth) {
		...
		gridWrapperWidth = Math.max(
			gridWrapperWidth,
			gridWrapper.getMathStyle("min-width")
		);
		gridWrapperWidth = Math.min(
			gridWrapperWidth,
			gridWrapper.getMathStyle("max-width")
		);
		...
		change.columns = Math.ceil(
			gridWrapperWidth / (MAX_COLUMN_WIDTH[this.props.context] || 250)
		);
	...
	endSplit(e) {
		...
		DBManager.edit(DBManager.entities.SETTINGS, {
			gridWidth: this.state.ui.gridWrapperWidth,
		});
	}
	...
	componentDidUpdate(prevProps) {
		...
		if (this.props.context != prevProps.context) {
			if (this.props.context == "GROUPS") this.applyUI(0);
	...
	render() {
		return (
			<div className={"library " + this.props.context}>
				...
				<ExportAsTextModal />
				<DownloadNotesProgress
					downloaded={this.props.downloadProgress.downloaded}
					total={this.props.downloadProgress.total}
				/>
	...
	renderStructure() {
		...
		return (
			<div
				className={
					"structure" +
					(this.props.collapsedStructure ? " collapsed" : "")
				}
				style={styles}
			>
				<Structure />
				<div className="collapser">
					<ToggleIcon onClick={::this.toggleStructure} />
				</div>
			</div>
		);
	}
	...
	renderNotes() {
		...
		let styles = {
			width: this.state.ui.gridWrapperWidth + "px",
			left: this.state.ui.structureWidth + "px",
		};
		...
		return (
			<div
				className="notes"
				style={styles}
				ref={(node) => (this.gridWrapper = node)}
			>
				{(() =>
					this.props.context == "SEARCH" ? (
						<SearchResultSet ref="contentWrapper" />
					) : (
						<Notes ref="contentWrapper" />
					))()}
				<div className="splitter" onPointerDown={::this.beginSplit}>
					<Dot />
				</div>
			</div>
		);
	...
	renderPreview() {
		...
		let styles = {
			width: this.state.ui.previewWidth + "px",
			left:
				this.state.ui.structureWidth +
				this.state.ui.gridWrapperWidth +
				"px",
		};
