...
import { triggerSaveNote as triggerEditSaveNote } from "../actions/edit";
...
import { triggerSaveNote as triggerLiveSaveNote } from "../actions/live";
import {
	closeDialog,
	moveWizardTo,
	openDialog,
...
class App extends Component {
	...
	componentDidMount() {
		...
		AppManager.triggerEditSaveNote = this.props.triggerEditSaveNote;
		AppManager.triggerLiveSaveNote = this.props.triggerLiveSaveNote;
		...
		NativeLinker.linkStoreUpdate((message) => {
			if (
				([ActionTypes.TAGS_UPDATE, "UPDATE_NOTES"].includes(
					message.action
				) ||
					(message.action == ActionTypes.CLOUD_SYNCING &&
						!message.body)) &&
				AuthenticationManager.profile &&
				!AuthenticationManager.profile.linkedWithCloud
			) {
				DBManager.edit(DBManager.entities.PROFILE, {
					linkedWithCloud: true,
				}).then(() => {
			...
			if (message.action == "UPDATE_NOTES") {
				...
				ContentManager.edit(
					message.body.map((data) => Note.fromJSON(undefined, data))
				);
				...
				if (entity.type == "groups") {
					if (
						...
						!entity.get(this.props.filterGroup)
					) {
						let onUpdate = ContentManager.onUpdate;
				...
				} else ContentManager.setEntity(entity);
			} else if (message.action == "CLOUD_FIRST_SYNC_COMPLETED")
				ContentManager.cleanEntityRelations();
			else this.props.dispatch(ActionTypes[message.action], message.body);
		});
		...
		if (this.props.update) this.props.completeAppUpdate();
	}
