    ...
    if (this.config.buttons) {
      this.config.buttons.forEach((button, i) => {
        if (typeof button.click == "function")
          ...
          else button.click = this.props[button.click].bind(this);
        }
            ...
            <div className="right-pane">
              <div className="flex-wrapper">
                <div className="fte-content">
                  {this.config.title ? (
                    <h1>
                      <FormattedMessage id={this.config.title} />
                    </h1>
                  ) : (
                    ""
                  )}
                  {this.config.content ? (
                    <p>
                      <FormattedMessage id={this.config.content} />
                    </p>
                  ) : (
                    ""
                  )}
                      ...
                      return (
                        <div className="action-bar">
                          <div className="flex-row">
                            {this.config.buttons
                              .filter((b) => b.row === undefined)
                              .map((button, i) => (
                                <Button
                                  key={this.config.type + i}
                                  {...button}
                                />
                              ))}
                          </div>
