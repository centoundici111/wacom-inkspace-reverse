...
class Menu extends Component {
	...
	navigateToPage() {
		if (ContentManager.selected.length == 1) this.props.editNote();
	}
	...
	render() {
		let found =
			ContentManager.getSelectedNotes().filter(
		...
		let iconClasses = className({
			...
			disabled:
				...
				this.props.rotateInProgress ||
				this.props.context == "GROUPS",
		});
		...
		let exportIconClasses = className({
			...
			disabled:
				...
				this.props.rotateInProgress ||
				(this.props.context == "GROUPS" && !this.props.filterGroup),
		});
			...
			<ul>
				<li>
					<Tooltip
						...
						overlay={
							<div>
								<FormattedMessage id={"tooltip.tags.manager"} />
							</div>
						}
						align={{ offset: [0, 10] }}
					>
						<a
							className={tagsManagerIconClasses}
							onClick={this.openTagsManager.bind(this)}
						>
							<AddTagIcon />
						</a>
					</Tooltip>
				...
				<li>
					<Tooltip
						...
						overlay={
							<div>
								<FormattedMessage id={"tooltip.rotate"} />
							</div>
						}
						align={{ offset: [0, 10] }}
					>
				...
				<li>
					...
					>
						<a
							...
							onClick={() => contextMenuManager.showExportMenu()}
						>
							<ExportIcon />
						</a>
					</Tooltip>
