...
class Note extends Component {
	...
	selectItem(e) {
		...
		if (ctrl) {
			...
			if (this.state.selected) {
				if (selectedIDs.length > 1) selectedIDs.remove(this.props.id);
			} else selectedIDs.push(this.props.id);

			if (ContentManager.selected.length != selectedIDs.length)
				this.props.selectNotes(selectedIDs);
		} else this.props.selectNotes(this.props.id);
	}
	...
	dragStart(e) {
		...
		if (
			...
			(ContentManager.selected.length == 1 &&
				ContentManager.selected.first != e.target.id)
		) {
			e.preventDefault();
			e.stopPropagation();
	...
	onContextMenu(e) {
		...
		if (!this.state.selected) this.props.selectNotes(this.props.id);
	...
	static getDerivedStateFromProps(props, state) {
		...
		if (props.rotatedNotes.includes(props.id)) update.preloader = true;
	...
	render() {
		...
		if (
			...
			this.props.editedNotes.includes(this.props.id)
		) {
			loader = <LoadingIcon />;
			onDoubleClick = null;
			onContextMenu = null;
		...
		return (
			...
			>
				<div className="thumb background-image" style={image}>
					{loader}
				</div>
	...
	componentDidUpdate(prevProps, prevState) {
		if (this.state.shouldScroll) {
			...
			this.props.filterGroup != prevProps.filterGroup &&
			ContentManager.selected.first == this.props.id
		)
