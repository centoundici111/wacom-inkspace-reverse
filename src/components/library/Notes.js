...
class Notes extends Component {
	constructor(props) {
		...
		Object.defineProperty(this, "handleScroll", {
			value: utils.debounce(
				() =>
					this.isLibraryEmpty() || !this.refs.list
						? null
	...
	openTutorial(e) {
		...
		if (DeviceManager.device) this.props.openWizard(WizardTypes.TUTORIAL);
		else
	...
	handleKeyDown(e) {
		if (
			this.props.searchWindowVisible ||
			DeviceManager.context == DeviceManager.Context.SETUP ||
			this.props.modal ||
			ContentManager.sections.length == 0
		)
			return;
		...
		if (Object.values(arrow).includes(e.keyCode)) {
			...
			if (kbSelector == KB_SELECTOR_DEFAULT) {
				let beginSection = ContentManager.sections.filter((section) =>
					section.includes(ContentManager.selected.first)
				).first;
				let endSection = ContentManager.sections.filter((section) =>
					section.includes(ContentManager.selected.last)
				).first;

				kbSelector = {
					firstSectionIDX:
						ContentManager.sections.indexOf(beginSection),
					firstSectionItemIDX: beginSection.indexOf(
						beginSection.filter((item) =>
							ContentManager.selected.includes(item)
						).first
					),
					lastSectionIDX: ContentManager.sections.indexOf(endSection),
					lastSectionItemIDX: endSection.indexOf(
						endSection.filter((item) =>
							ContentManager.selected.includes(item)
						).last
					),
				};
			...
			switch (e.keyCode) {
				...
				case arrow.down:
					let nextSectionIDX =
						kbSelector.lastSectionIDX +
						(e.keyCode == arrow.up ? -1 : 1);
				...
				case arrow.right:
					nextSection =
						ContentManager.sections[kbSelector.lastSectionIDX];

					let nextSectionItemIDX =
						kbSelector.lastSectionItemIDX +
						(e.keyCode == arrow.left ? -1 : 1);
					nextNote = nextSection[nextSectionItemIDX];

					if (!nextNote) {
						if (e.keyCode == arrow.left) {
							nextSection =
								ContentManager.sections[
									kbSelector.lastSectionIDX - 1
								];
			...
			if (nextNote) {
				...
				let nextSectionIDX =
					ContentManager.sections.indexOf(nextSection);
				let nextSectionItemIDX =
					ContentManager.sections[nextSectionIDX].indexOf(nextNote);
				...
				if (ctrl) {
					...
					firstSectionIDX = Math.min(
						kbSelector.firstSectionIDX,
						nextSectionIDX
					);
					lastSectionIDX = Math.max(
						kbSelector.firstSectionIDX,
						nextSectionIDX
					);
					...
					if (kbSelector.firstSectionIDX < nextSectionIDX) {
						...
						firstSectionItemIDX = Math.min(
							kbSelector.firstSectionItemIDX,
							nextSectionItemIDX
						);
						lastSectionItemIDX = Math.max(
							kbSelector.firstSectionItemIDX,
							nextSectionItemIDX
						);
					}
					...
					let firstNote =
						ContentManager.sections[firstSectionIDX][
							firstSectionItemIDX
						];
					let lastNote =
						ContentManager.sections[lastSectionIDX][
							lastSectionItemIDX
						];

					for (
						let i = orderedNotes.indexOf(firstNote);
						i <= orderedNotes.indexOf(lastNote);
						i++
					)
					...
					kbSelector = {
						firstSectionIDX: nextSectionIDX,
						firstSectionItemIDX: nextSectionItemIDX,
						lastSectionIDX: nextSectionIDX,
						lastSectionItemIDX: nextSectionItemIDX,
					};
				...
				this.keepNoteVisible(
					nextNote,
					e.keyCode == arrow.down || e.keyCode == arrow.right
				);
			}
		}
	...
	keepNoteVisible(noteID, bottom) {
		let node = Array.from(document.querySelectorAll(".grid .item")).filter(
			(item) => item.id == noteID
		).first;

		if (
			!Module.RectTools.intersect(
			...
			if (bottom)
				this.refs.content.scrollTop =
					...
					node.offsetHeight +
					node.getMathStyle("margin-bottom");
			else this.refs.content.scrollTop = node.offsetTop;
		}
	}
	...
	beginSelect(e) {
		if (this.ctrl) {
			let pin = {
				x: e.clientX - document.querySelector(".notes").offsetLeft,
				y: e.clientY + this.refs.content.scrollTop,
			};
	...
	moveSelect(e) {
		...
		let width =
			e.clientX -
			this.selector.pin.x -
			document.querySelector(".notes").offsetLeft;
		let height =
			e.clientY - this.selector.pin.y + this.refs.content.scrollTop;
	...
	endSelect(e) {
		...
		if (this.selectorMoved) this.props.refreshLibrary();
	}
	...
	componentDidUpdate(prevProps, prevState) {
		if (prevProps.filterGroup != this.props.filterGroup && this.refs.list) {
			if (this.refs.content.firstElementChild)
				this.refs.content.scrollTop =
					this.refs.content.firstElementChild.offsetHeight;
		}
	}
	...
	render() {
		return (
			<div
				className={
					"container" + (this.props.filterTag ? " filter" : "")
				}
			>
				{this.renderHeader()}
				{this.renderFilter()}
			...
			return (
				<div className="tags-filter">
					<div className="tag-filter">
						...
						<a
							onClick={(event) => this.props.filterByTag()}
	...
	renderEmptyLibrary() {
		return (
			<div className="flex-wrapper">
				<div className="empty">
					<h1>
						<FormattedMessage id={"empty.library.text"} />
					</h1>
					<a
	...
	renderEmptyGroup() {
		return (
			<div className="flex-wrapper">
				<div className="empty">
					<h1>
						<FormattedMessage id={"empty.group.text"} />
					</h1>
					<div className="details">
						<FormattedMessage id={"empty.group.details"} />
					</div>
					<div className="subdetails">
						-{" "}
						<FormattedMessage
							id={"empty.group.subdetails.drag.drop"}
						/>
					</div>
					<div className="subdetails">
						-{" "}
						<FormattedMessage
							id={"empty.group.subdetails.context.menu"}
						/>
					</div>
				</div>
			</div>
		...
		return (
			<div className="flex-wrapper">
				<div className="empty empty-filter">
					<h1>
						<FormattedMessage
							id={"library.empty.tags.filter.title"}
						/>
					</h1>
					<p>
						<FormattedMessage
							id={"library.empty.tags.filter.description"}
						/>
					</p>
				</div>
			</div>
	...
	renderGrid() {
		...
		if (!this.selector.pin) style.display = "none";
		else {
				...
				{(() => {
					if (this.isLibraryEmpty()) {
						if (this.props.filterTag)
							...
							return this.props.filterGroup
								? this.renderEmptyGroup()
								: this.renderEmptyLibrary();
					} else return this.renderSections();
				})()}
	...
	renderSections() {
		let selectedSection = ContentManager.sections.filter((section) =>
			section.includes(ContentManager.selected.first)
		).first;
		...
		return (
			<ReactList
				ref="list"
				itemRenderer={::this.renderSection}
				pageSize={pageSize}
				initialIndex={initialIndex}
				length={ContentManager.sections.length}
				scrollParentGetter={global.getScrollParent}
			/>
		);
	}
