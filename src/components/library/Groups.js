...
class Groups extends Component {
		...
		Object.defineProperty(this, "group", {
			get: () =>
				this.props.filterGroup
					? ContentManager.getEntity("groups").get(
							this.props.filterGroup
					  )
					: null,
		});
	...
	init() {
		...
		if (!this.props.filterGroup) {
			let group =
				ContentManager.getEntity("groups").get(
					this.props.lastSelectedGroup
				) || ContentManager.getEntity("groups").orderedValues.first;
	...
	componentDidMount() {
		if (this.props.filterGroup) this.setState({ shouldScroll: true });
	}
	...
	renderEmptyGroups() {
		...
		return (
			<div className="flex-wrapper">
				<div className="empty">
					<h1>
						<FormattedMessage id={"empty.groups.text"} />
					</h1>
	...
	renderGroup(group, i) {
		...
		let className =
			"item" +
			(this.group && this.group.id == group.id ? " selected" : "");

		if (note) image = { backgroundImage: `url("${note.getThumbSrc()}")` };
		...
		return (
			<div
				...
				key={"folder" + i}
				className={className}
				onClick={this.selectItem.bind(this, group)}
				onDoubleClick={this.open.bind(this, group)}
				onContextMenu={(event) => this.openContextMenu(event, group)}
			>
				<div className="thumb background-image" style={image}></div>
				<div className="thumb-title">{group.name}</div>
