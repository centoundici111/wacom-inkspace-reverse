...
class Preview extends Component {
	...
	render() {
		let rotating =
			this.props.rotatedNotes.length &&
			ContentManager.isSelected(this.props.rotatedNotes.first);
		let editing =
			this.props.editedNotes.length &&
			ContentManager.isSelected(this.props.editedNotes.first);

		let loaderStyle =
			this.props.combining || rotating || editing
				? null
				: { display: "none" };
		...
		if (ContentManager.selected.length == 0) wrapperClasses += " empty";
			...
			if (this.props.combining)
				...
				details = (
					<FormattedMessage
						id={"preview.items.selected"}
						values={{ count: ContentManager.selected.length }}
					/>
				);
		} else wrapperClasses += " single";
		...
		if (rotating || editing) {
			...
			if (rotating)
				details = (
					<FormattedMessage id={"preview.rotate.in.progress"} />
				);
			else if (editing)
							...
							// ContentManager.selected.length == 1
								...
								return (
									<div
										...
										style={{
											backgroundImage: `url("${ContentManager.getNote(
												ContentManager.selected.first
											).getPreviewSrc()}")`,
										}}
										onDoubleClick={this.props.editNote}
