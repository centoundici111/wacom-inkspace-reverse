...
import EraserIcon from "../icons/edit/EraserIcon.svg";
...
class Menu extends Component {
	...
	componentDidMount() {
		...
		if (
			...
			this.props.note.lastModifiedDate != this.props.lastModifiedDate
		)
	...
	render() {
		...
		var selectorIconClass = classNames({
		...
		var splitIconClass = classNames({
			icon: true,
			disabled:
				this.props.note.strokes.length < 2 || this.props.noteProgress,
		});
				...
				<div className="menu left pull-left">
					<ul>
						<li>
							...
							>
								<a
									className={backToLibraryClass}
									onClick={this.props.transferToLibrary}
								>
									<HomeIcon />
								</a>
							</Tooltip>
						...
						<li>
							<Tooltip
								...
								overlay={
									<div>
										<FormattedMessage id={"tooltip.save"} />
									</div>
								}
								align={{ offset: [0, 10] }}
							>
						...
						<li>
							...
							>
								<a
									...
									onClick={this.props.initNoteSplit}
								>
									<SplitNoteIcon />
								</a>
							</Tooltip>
					...
					<ul>
						<li className={penIconClass}>
							...
							>
								<a
									className="icon"
									onClick={this.props.selectTool.bind(
										{},
										"pen"
									)}
								>
					...
					<ul>
						<li className={layerIconClass}>
							...
							>
								<a
									className="icon"
									onClick={this.props.toggleLayerPanel}
								>
									<LayersIcon />
								</a>
							</Tooltip>
...
function mapStateToProps(state) {
	return {
		...
		layerPaneVisible:
			state.EditReducer.layerPaneVisible &&
			state.EditReducer.splitMode != "note",
