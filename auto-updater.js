...
let AutoUpdater = {
	init() {
		...
		// Local Electron runs are not packaged or code-signed, so updater startup
		// will fail before the app window can open.
		if (!app.isPackaged) {
			this.updateState(AutoUpdater.State.NOT_FOUND)
			return
		}
